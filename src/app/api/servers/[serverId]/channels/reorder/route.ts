...
export async function POST(
  ...
  try {
    ...
    const { items } = await request.json() as { items: { id: string; position: number; categoryId?: string | null }[] }
    if (!Array.isArray(items)) return NextResponse.json({ error: 'items[] required' }, { status: 400 })

    await prisma.$transaction(items.map((it) =>
      prisma.channel.update({ where: { id: it.id }, data: { position: it.position, ...(it.categoryId !== undefined ? { categoryId: it.categoryId } : {}) } as any })
    ))
