...
// PATCH: mute or unmute a member in a server
// body: { durationMinutes?: number } if omitted or 0 => unmute
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ serverId: string, userId: string }> }) {
  ...
  try {
    ...
    const mutedUntil = durationMinutes && durationMinutes > 0
      ? new Date(Date.now() + durationMinutes * 60 * 1000)
      : null

    const data: any = { mutedUntil }
    ...
    return NextResponse.json({ error: 'Failed to mute/unmute' }, { status: 500 })
  }
}
